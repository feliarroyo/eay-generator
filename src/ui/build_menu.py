...
class BuildMenuWidget(QWidget):
    ...
    def build_episode(self, include_base_prompts, selected_episodes, build_path=None):
        ...
        if success:
            QMessageBox.information(self, self.tr("Success"), f"Files generated successfully for {build_type}.")
        else:
            QMessageBox.critical(self, self.tr("Error"), f"Build failed for {build_type}.")
    
    def open_revert_menu(self):
        self.revert_menu = RevertMenuWidget(self.parent_window)
        self.revert_menu.show()
        self.setEnabled(False)
        self.revert_menu.closeEvent = lambda event: self.setEnabled(True)