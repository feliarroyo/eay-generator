...
root_project_dir = get_user_data_path()
print(f"DEBUG: App is using this root directory: {root_project_dir}")
episodes_path = root_project_dir / "episodes"
backup_path = root_project_dir / "backup"
app_build_path = root_project_dir / "build"
temp_path = root_project_dir / ".temp_session"
...
def registerPack(path):
    ...
    with open(root_project_dir / f"game_pack_{pack_number}.txt", 'w') as f:
        f.write(path)
...
def get_linked_game_pack_path(pack_number):
        ...
        path_file = root_project_dir / f"game_pack_{pack_number}.txt"
...
def generateFibbageFiles(selected_episodes, base_folder_structure, shortieFileType, dataFileType, fileName, build_path=app_build_path, include_base_prompts=False):
    ...
    for episode in selected_episodes:
        for prompt in read_episode_prompts(episode.text()):
            ...
            prompt_path = target_path / fileName / str(prompt_index)
            os.makedirs(prompt_path, exist_ok=True)
            
            # Add audio file if needed
            if prompt.has_audio():
                audio_path = episodes_path / episode.text() / prompt.audio
                if os.path.exists(audio_path):
                    shutil.copy2(audio_path, prompt_path / "questionAudio.ogg")
                else:
                    QMessageBox.warning(None, "Warning", f"Audio file for prompt '{prompt.personal_question}' not found.")
            
            # Create data.jet file
            data_jet_path = prompt_path / "data.jet"
